...
class sdbh {
	...
	function __construct($force_master = false){
		$this->port = 3306;
		$this -> host = 'localhost';
		$this -> dbname = 'u2343678_test_a25';
		$this -> user = 'u2343678_default';
		$this -> pass = '64H7rHfQRRNGp6No';
    ...
    )
    {
        ...
        while (True) try {
            $where_clause = !empty($select_array) ? "WHERE " . $this->make_str($select_array, 'AND', $tbl_name) : "";
